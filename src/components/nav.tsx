...
function NavItem({
  ...
  return (
    <Link
      href={href}
      className={cn(
        "relative cursor-pointer rounded-full px-3 py-1 text-sm font-medium transition-colors",
        active
          ? "text-accent-brand"
...
export function Nav() {
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-4 z-50 flex justify-center px-4 sm:top-6">
      <nav className="pointer-events-auto flex items-center gap-1 rounded-full border border-border bg-background/80 px-2 py-1.5 shadow-lg shadow-black/20 backdrop-blur-md">
        <Link
          href="/"
        ...
        <a
          ...
          className="inline-flex cursor-pointer items-center gap-1 rounded-full px-3 py-1 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
        >
          <span>Fiverr</span>
